  ...
  services.push({
    ...
    data: {
      ...
      env: [
        ...
        input.smtpReplyEmail ? `SMTP_REPLY_EMAIL=${input.smtpReplyEmail}` : "",
        input.smtpTlsCiphers ? `SMTP_TLS_CIPHERS=${input.smtpTlsCiphers}` : "",
        `SMTP_SECURE=${input.smtpSecure}`,
        input.smtpName ? `SMTP_NAME=${input.smtpName}` : "",
      ].join("\n"),
      source: {
        type: "image",
